...
You ask your favorite LLM for a joke about coffee. You ask again. You get the same joke, no matter which model you try. You ask for a story about a bear, and it always begins with "Once upon a time, in a land far away..." The brainstorming ideas feel generic, the outputs repetitive.
...
**Research & AI-Driven Discovery** [@si2024llmsgeneratenovelresearch]: Perhaps most critically, mode collapse hampers AI's role in scientific discovery and research ideation. When researchers use LLMs to generate hypotheses, explore experimental designs, or brainstorm research directions, they need the full spectrum of possibilities, including approaches that might lead to breakthroughs. Mode collapse means the AI suggests only well-trodden paths, missing potentially transformative ideas that lie in the less-typical regions of its knowledge.
...
We start with an intuitive example: The Library Metaphor, alongside mathematical proofs that rigorously showing why each approach can produce different distributions. Imagine a massive library and the LLM as the librarian:

- **Direct Prompt** (Example Prompt: "tell me a coffee joke"): The librarian walks straight to the "Most Popular" shelf and hands you the same book every time. This is mode collapse.
...
- **List Prompt** (Example Prompt: "tell me 5 coffee jokes"): The librarian goes to one aisle and grabs the first five books they see. You get variety, but limited to one section.
...
- **Verbalized Sampling** (Example Prompt: "tell me 5 coffee jokes with their probabilities"): You're asking the librarian to first describe the entire library's collection: mystery, SciFi, history, all of it, and then pick five random books that represent that whole collection.
...
### Comparing VS against Baseline Methods

We compare VS against several baseline prompting strategies. All methods use the same computational budget of $N$ total responses, with $k=5$ candidates per call for multi-candidate methods.<Sidenote number={4}>We empirically found $k=5$ provides the best quality-diversity tradeoff. Ablation studies for different values of $k$ can be found in Appendix H.1 of our paper.</Sidenote>

<Table id="methods-comparison" caption="Comparison of prompting methods evaluated in our experiments">
  <thead>
    <tr>
      <th>Category</th>
      <th>Method</th>
  ...
  <tbody>
    <tr>
      ...
      <td>Direct</td>
      <td>Standard prompting for a single response</td>
      <td>"Tell a joke about coffee"</td>
    </tr>
    <tr>
      <td>CoT</td>
      <td>Add CoT to Direct Prompt</td>
      <td>"Think step-by-step, then tell a joke"</td>
    </tr>
    <tr>
      <td rowspan="2"><strong>List-level</strong></td>
      <td>Sequence</td>
      <td>Requests multiple responses in one call</td>
      <td>"Tell 5 jokes about coffee"</td>
    </tr>
    <tr>
      <td>Multi-Turn</td>
      <td>Repeatedly asks for "another" candidate across turns</td>
      <td>"Turn 1: Tell a joke about coffee"<br/>Turn 2+: Tell another"</td>
    </tr>
    <tr>
      <td rowspan="3"><strong>Distribution-level (Ours)</strong></td>
      <td><strong>VS-Standard</strong></td>
      <td>Requests multiple responses with their probabilities</td>
      <td>"Tell 5 jokes with their probabilities"</td>
    </tr>
    <tr>
      <td><strong>VS-CoT</strong></td>
      <td>VS-Standard + CoT</td>
      <td>"Think step-by-step, then tell 5 jokes with probabilities"</td>
    </tr>
    <tr>
      <td><strong>VS-Multi</strong></td>
      <td>VS-Standard + Multi-turn</td>
      <td>Turn 1: "Tell 2 jokes with probabilities"<br/>Turn 2+: "Tell another 2 with probabilities"</td>
    </tr>
  </tbody>
</Table>
...
**Settings.** We evaluate our methods against baseline methods on three creative writing tasks: (1) **Poem continuation**, (2) **Story generation**, and (3) **Joke writing**. For evaluation, we measure both diversity and quality: **Semantic Diversity** (Embedding-based diversity) and **Quality** (LLM-as-Judge with creative or funniness rubrics).<Sidenote number={5}>We acknowledge that LLM-as-Judge can be biased and inaccurate on truly understanding **creativity**. We leave it to future work to develop more fine-grained evaluation metrics.</Sidenote>
...
VS variants achieve **Pareto-optimal diversity-quality tradeoffs** (Figure 1d), with VS-Multi reaching the highest diversity while maintaining quality compared to baseline methods like Direct and Sequence prompting.

We observe an emergent trend where **larger models benefit more from VS** (Figure 1e-f). Across all VS variants, larger models (GPT-4.1, Gemini-2.5-Pro) achieve diversity gains **1.5 to 2 times greater** than smaller models (Figure 1e). Smaller models (GPT-4.1-mini, Gemini-2.5-Flash) show more pronounced quality drop, whereas larger models maintain or improve quality (Figure 1f).
...
<Figure id="temperature-ablation" caption="VS is orthogonal to temperature; combining the two improves the diversity–quality frontier.">
  <img src="/images/temperature_scaling.png" alt="Temperature scaling ablation studies between direct, sequence, and vs-standard. We test on GPT-4.1 and Gemini-2.5-Flash models." style="border: none; box-shadow: none; max-width: 80%; height: auto;" />
</Figure>
...
The results demonstrate that it is possible to **combine VS with temperature to further push the diversity**. At any given temperature, VS consistently achieves better diversity than baseline methods, and the combination enables reaching optimal points that neither approach achieves alone.
...
Besides simply creativity task, we also evaluate VS on synthetic data generation to test its effectiveness in boarder domains. Recent research has shown that the diversity of synthetic data plays an important role in improving downstream model performance [@zhu2025bareleveragingbaselanguage].
...
**Fine-tuning.** With this 1K synthetic dataset, we follow the SFT setting in LIMO [@ye2025limoreasoning], an effective method to improve reasoning performance with small dataset size, and finetune three models on this dataset: Qwen2.5-7B, Qwen3-1.7B-Base, and Qwen3-4B-Base.

**Evaluation.** We evaluate the fine-tuned models' downstream task performance on three widely used math benchmark datasets: MATH500, OlympiadBench, and Minerva Math, which cover a wide range of topics, including algebra, geometry, and competitive mathematics.

<Table id="synthetic-results" caption="Downstream accuracy averaged across MATH500, OlympiadBench and Minerva Math. The results are average across three models and three datasets. VS and its variants improve downstream tasks." captionSize="xs">
  <thead>
    <tr>
      <th>Method</th>
...
This isn't just creative, it's **thoughtful**. Bernard can be read as:
...
- A metaphor for the LLM itself: a vast, creative mind (the bear) constrained by alignment training (the suit and tie), but still, fundamentally, a bear.

This shows it's possible to enhance the diversity of the LLMs instead of receiving the most common story, there are just more possibilities under LLM's true distribution waiting to be explored.
