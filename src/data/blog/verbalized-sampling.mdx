...
This frustrating phenomenon is **mode collapse**. Past research blamed the AI's post-training process (e.g., RLHF), assuming the algorithms naturally favored the most common, "safe" answer.

**We discovered something more fundamental**: The problem isn't just the AI. It's us.

## 2. Intuition

Aligned LLMs collapse to stereotypical outputs due to **typicality bias** in human preferences. VS asks for a small distribution (k items + probabilities) to recover diversity without training.
...
With a standard prompt for "write a short story about a bear," we get a generic tale about a bear in a forest. It's fine, but forgettable.

With Verbalized Sampling, we get this:
