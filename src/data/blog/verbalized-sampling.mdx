...
## 2. How It Works: From Intuition to Theory
...
### 2.4 The Solution: Distribution-Level Recovery
...
On the US states task, VS aligns closely with the corpus distribution ($\mathrm{KL}\approx 0.12$), while direct prompting shows severe collapse [@zhang2025vs, p. 3].
...
**Key insight**: VS asks for $k$ candidates with probabilities, then uses a threshold $\tau$ to tune diversity—lower thresholds include more diverse, lower-probability outputs [@zhang2025vs, pp. 7–8].
